...
public class Ex_07_26_Method2 {
	...
	public static void main(String args[]) {
		 ...
		 * 
		 * sort(score, student2, name); print(name, sub_name, score,subject2, student2);
		 */
	} // main() end
	...
	private static void sort(int[][] score, int[] student, String[] name) {
		...
		for (int i = 0; i < score.length - 1; i++) {
			for (int j = i + 1; j < score.length; j++) {
				if (student[i] < student[j]) { // 부등호 방향 바꾸면 내람차순 , 올림차순 으로 변한다.
					String temp = name[i];
					name[i] = name[j];
					name[j] = temp;
	...
	/*
	 * private static int[][] getTotal(int[][] score) { int[][] total = { {0,0,0},
	 * // 과목 총점 {0,0,0,0,0} // 학생 총점 }; for(int i = 0; i < score.length; i++) { //학생
	 * for(int j = 0; j < score[i].length; j++ ) { //과목 total[0][j] += score[i][j];
	 * total[1][i] += score[i][j]; } } return total; }
	 */
}
