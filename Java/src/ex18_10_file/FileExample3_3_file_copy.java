...
public class FileExample3_3_file_copy {
	...
	private static void copy(String originalFileName, String targetFileName) {
		...
		{
			byte[] readByte = new byte[1024]; // 1024byte 씩 읽어 들어올 byte 배열 생성 

			while (in.read(readByte) != -1) { // 생성한 byte 배열에 파일 데이터를 byte 로 저장, 더이상 읽어들일 데이터 없으면 -1 반환
				out.write(readByte); // in.read가 -1을 반환하지 않는 동안 
				//배열 readByte 에 있는 byte 데이터를 전송할 파일 위치에 전송 및 파일 생성
			}
