...
public class FileExample3_4_Buffered {
	...
	private static void copy(String originalFileName, String targetFileName) {
		try (BufferedInputStream in = new BufferedInputStream(new FileInputStream(originalFileName));
		...
		// try with resource 사용해서 FileInputStream(읽어올 파일 위치) 과 FileOutputStream(전송할 파일 위치) 객체 생성
		{
			...
			// in = new FileInputStream 과 out = new FileOutputStream 객체 생성할 때 일어나는
