...
public class JDBCExample2_Oracle1_column_name_emp2 {
	...
	public static void main(String args[]) throws Exception {
		...
		try {
			...
			String select_sql = "select empno, ename, job, mgr, hiredate, sal, comm, deptno from emp";
			// select 문 문자열
			...
			while(rs.next()) { // 더 이상 읽을 데이터가 없을 때까지 반복
				...
				String ename = rs.getString("ename"); // the column value; if the value is SQL NULL, the value returned is null
				String job = rs.getString("job");
				int mgr = rs.getInt("mgr");
				
				
				//java.sql.Date hiredate = rs.getDate("hiredate"); // 1980-12-17
				//String hiredate = rs.getString("hiredate"); //1980-12-17 00:00:00 처럼 시분초까지 출력되므로 substring(0,10); 해준다.
				String hiredate = rs.getString("hiredate").substring(0,10); 
				//날짜 데이터도 자동으로 String 으로 가져온다. 대신 날짜 데이터 전체가 출력되므로 필요한 부분만 잘라줘야 한다.
				...
				//null 일 경우 0으로 자동으로 가져온다. => nvl 안해도 된다.
