...
public class JDBCExample2_Oracle1_column_index_dept {
	...
	public static void main(String args[]) throws Exception {
		...
		try {
			...
			while(rs.next()) { // 더 이상 읽을 데이터가 없을 때까지 반복
				...
				String loc = rs.getString(1); // 조회 시 첫번째 컬럼의 값을 가져온다.
				/*
				 ...
				 */
				int deptno = rs.getInt(2); // 조회시 두번째 컬럼의 값을 가져온다.
				String dname = rs.getString(3); // 조회시 세번째 컬럼의 값을 가져온다.
